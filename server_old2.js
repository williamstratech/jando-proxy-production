...
var server1_handler = function (req, res) {
  var need_authentication = function(){
    ...
    //whitelist Cyprus office
    if(req.headers['cf-connecting-ip'] == '91.184.200.174'){
      return false;
    };
    //whitelist Stellenbosh office
    if(req.headers['cf-connecting-ip'] == '196.213.190.250'){
      return false;
    };
  ...
  if(target.length == 0 && req_obj.headers.host == 'secure-stage.jando.com'){
  ...
  if(target.length == 0 && req_obj.headers.host == 'cms-dev.jando.com'){
    target = 'http://' + wordpress;
  }
  ...
  if(target.length == 0 && req_obj.headers.host == 'business-stage.jando.com'){
    target = 'http://' + demo_server;
  }

  //from here is defaults to staging.jando.com  (in future now secure-stage.jando.com)
